...
export default function Header({ onMenuClick }) {
  ...
  const pageTitle = pathname?.split("/").pop() || "scan";
  const formattedTitle = pageTitle === "scan" ? "SecOps Terminal" : pageTitle.charAt(0).toUpperCase() + pageTitle.slice(1);
    ...
    <header className="sticky top-0 z-40 w-full glass-header transition-all duration-300">
      <div className="flex h-16 items-center justify-between px-6 md:px-8">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="h-9 w-9 rounded-full bg-gradient-to-tr from-primary to-emerald-600 flex items-center justify-center text-xs font-bold text-white shadow-lg ring-2 ring-transparent hover:ring-primary/30 transition-all cursor-pointer overflow-hidden">
            {user?.photoURL ? (
